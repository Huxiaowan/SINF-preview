...
from utils.core import open_sequence
from utils.core import list_dir, open_images_uint8
...
class SrgbTrainDataset(Dataset):
    def __init__(self, seq_dir, train_length, patch_size, patches_per_epoch, temp_stride=3, image_postfix='png', pin_memory=False):
        ...
        for seq_name in self.seq_names:
            ...
            # print(list_dir(os.path.join(self.seq_dir, seq_name),)
            if self.pin_memory:
...
class SrgbValDataset(Dataset):
    ...
    def __getitem__(self, index):
        return_dict = {'seq': torch.from_numpy(self.sequences[index]), 'name': self.seqs_dirs[index]}
        # crop patches
        data = return_dict['seq']
        num_frames, C, H, W = data.shape

        # data = data[:, :, :int(H//32*32), :int(W//32*32)]
        return_dict['seq'] = data
        return return_dict
